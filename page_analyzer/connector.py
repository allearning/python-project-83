...
class Connector:
    ...
    def get_page(self, name: str):
        with self.connection.cursor() as cursor:
            cursor.execute(
                'SELECT name, id, created_at FROM urls WHERE name = %s',
                (name,))
            try:
    ...
    def get_page_by_id(self, page_id):
        with self.connection.cursor() as cursor:
            cursor.execute(
                'SELECT name, id, created_at FROM urls WHERE id = %s',
                (page_id,))
            try:
                data = cursor.fetchone()
            except psycopg2.ProgrammingError as error:
    ...
    def get_last_check(self, page_id):
        with self.connection.cursor() as cursor:
            cursor.execute(
                '''SELECT url_id, status_code, h1, title,
